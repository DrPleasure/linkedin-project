...
export const getUsersAction = () => {
  return async (dispatch) => {
    ...
    try {
      let resp = await fetch(baseEndPoint, getOptions);
      if (resp.ok) {
...
export const getMyProfileDetailsAction = (userId) => {
  ...
  return async (dispatch) => {
    ...
    try {
      let response = await fetch(baseUrlMe, getOptions);
      if (response.ok) {
        let data = await response.json();
...
export const changeProfileDetailsAction = (details, id) => {
  ...
  return async (dispatch) => {
    ...
    try {
      let response = await fetch(baseEndPoint + `${id}`, optionsPut);
      if (response.ok) {
        console.log("Profile Details sucessfully updated ->", response);
        const data = await response.json();
        console.log("🚀 ~ file: index.js:187 ~ return ~ data", data);
      } else {
      ...
      if (resp.ok) {
        ...
        fetchedPosts.forEach((post) => {
          post.likes = post.likes.map((like) => {
            like.user = like.userData; // Access the name property of the user object
            return like;
          });
        });
