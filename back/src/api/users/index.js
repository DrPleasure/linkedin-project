...
router.get("/pagination", async (req, res, next) => {
  try {
    ...
    const users = await Users.find(mongoQuery.criteria, mongoQuery.options.fields)
...
// FILTER by user.name
router.get("/filter", checkFilteredUserSchema, triggerBadRequest, async (req, res, next) => {
  try {
    const { name, email } = req.query;

    const filteredUsers = await Users.find({ name, email }, { title: 1, bio: 1 });

    if (filteredUsers.length > 0) {
      res.send(filteredUsers);
    } else {
      next(createHttpError(404, `There are no users that match these 2 criteria: name='${name}' AND email='${email}'`));
    }
  } catch (error) {
    next(error);
  }
...
router.delete("/:userId", async (req, res, next) => {
  try {
    ...
    if (deletedUser) {
      res.send({ message: `The user with id: ${userId} has been successfully deleted` });
    } else {
